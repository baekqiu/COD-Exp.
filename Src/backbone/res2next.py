...
class Res2NeXt(nn.Module):
    def __init__(self, block, baseWidth, cardinality, layers, num_classes, scale=4):
        ...
        self.layer3_1 = self._make_layer(block, 256, layers[2], 2)
        self.layer4_1 = self._make_layer(block, 512, layers[3], 2)

        self.inplanes = 512
        self.layer3_2 = self._make_layer(block, 256, layers[2], 2)
        self.layer4_2 = self._make_layer(block, 512, layers[3], 2)
...
def res2next50(pretrained=False, **kwargs):
    ...
    """
    model = Res2NeXt(Bottle2neckX, layers = [3, 4, 6, 3], baseWidth = 26, cardinality=1, scale = 4, num_classes=1000)
    return model
