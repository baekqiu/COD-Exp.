...
class SINet_ResNet50(nn.Module):
    ...
    def __init__(self, channel=32, opt=None):
        ...
        #self.add_branch = nn.Sequential(
        #    BasicConv2d(128, 512, 1),
        #)
        self.add_relu = nn.ReLU(True)
        self.add_conv_res = BasicConv2d(512, 512, 1)
    ...
    def forward(self, x):
        ...
        camouflage_map_sm ,camouflage_ma_to_aic= self.pdc_sm(x4_sm_rf, x3_sm_rf, x2_sm_rf, x01_sm_rf)
        


        # ----Add Module ----

        add_map = self.AIC(camouflage_ma_to_aic.sigmoid())       #(128,44,44)
        add_x0 = self.add_branch(add_map)              #(512,44,44)
        #print(add_x0)
        
        
        # ---- Switcher: Search Attention (SA) ----
        #print(camouflage_map_sm.sigmoid().shape)
        #print(camouflage_map_sm.size())
        x2_sa = self.SA(camouflage_map_sm.sigmoid(), x2)    # (512, 44, 44)
        #print(x2_sa)

        # ----add_x0 ,x2_sa=>add
        x2_sa = self.add_relu(self.add_conv_res(x2_sa + add_x0))
        #print(x2_sa)
