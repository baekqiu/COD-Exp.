...
from torchvision.models import res2net
from .SearchAttention import SA
from Src.backbone.res2next import res2next50
import torch.utils.model_zoo as model_zoo
from Src.backbone.res2net import res2net50_26w_4s
...
class SINet_Res2Net50(nn.Module):
    # resnet based encoder decoder
    def __init__(self, channel=32, opt=None):
        super(SINet_Res2Net50, self).__init__()
        ...
        self.edge_conv1 = BasicConv2d(128, 128, 3, padding=1)
        self.edge_conv2 = BasicConv2d(128, 128, 3, padding=1)
        self.edge_conv3 = nn.Conv2d(128, 1, 1)
    ...
    def forward(self, x):
        ...
        # - low-level features
        x0 = self.resnet.maxpool1(x0)    # (BS, 64, 88, 88)
        ...
        edge_1 = torch.cat((x01_sm_rf, x2_sm_rf, self.upsample_2(x3_sm_rf), self.upsample_2(self.upsample_2(x4_sm_rf))), dim=1) #44*44*128
        edge_2 = self.edge_conv3(self.edge_conv2(self.edge_conv1(edge_1))) 
        ...
        x2_im_rf_edge = torch.add(x2_im_rf, edge_2)
        x3_im_rf_edge = torch.add(x3_im_rf, self.downSample(edge_2))
        x4_im_rf_edge = torch.add(x4_im_rf, self.downSample(self.downSample(edge_2)))
        # - decoder
        camouflage_map_im = self.pdc_im(x4_im_rf_edge, x3_im_rf_edge, x2_im_rf_edge)#, x_low_im_rf
        ...
        new_x2_im_rf_edge = torch.add(new_x2_im_rf, edge_2)
        new_x3_im_rf_edge = torch.add(new_x3_im_rf, self.downSample(edge_2))
        new_x4_im_rf_edge = torch.add(new_x4_im_rf, self.downSample(self.downSample(edge_2)))
        camouflage_map_em = self.pdc_em(new_x4_im_rf_edge, new_x3_im_rf_edge, new_x2_im_rf_edge)

        # ---- output ----
        return self.upsample_8(camouflage_map_sm), self.upsample_8(camouflage_map_im), self.upsample_8(camouflage_map_em), self.upsample_8(edge_2)#
